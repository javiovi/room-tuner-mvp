...
export function RoomModesChart({ modes }: RoomModesChartProps) {
  ...
  const getColor = (severity: RoomMode["severity"]) => {
    switch (severity) {
      ...
      case "low": return isDark ? "#FF9F0A" : "#FF9500"
      default: return isDark ? "#FF9F0A" : "#FF9500"
    }
  }

  return (
    <div className="bg-card rounded-2xl card-shadow border border-border/50 p-5 space-y-3">
      <h2 className="text-sm font-semibold text-foreground">Modos de sala</h2>
      ...
      <div className="w-full h-64 mt-4">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={displayModes} margin={{ top: 5, right: 20, left: 0, bottom: 20 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={colors.grid} vertical={false} />
            <XAxis
              dataKey="frequency" stroke={colors.axis}
              style={{ fontSize: '10px', fill: colors.axis }}
              label={{ value: 'Frecuencia (Hz)', position: 'insideBottom', offset: -15, style: { fontSize: '10px', fill: colors.axis } }}
              tickFormatter={(value) => Math.round(value).toString()}
            />
            <YAxis
              stroke={colors.axis}
              style={{ fontSize: '10px', fill: colors.axis }}
              label={{ value: 'Intensidad', angle: -90, position: 'insideLeft', style: { fontSize: '10px', fill: colors.axis } }}
              domain={[0, 3]} ticks={[0, 1, 2, 3]}
              tickFormatter={(value) => { const labels = ["", "Baja", "Media", "Alta"]; return labels[value] || "" }}
            />
      ...
      <div className="grid grid-cols-3 gap-2 mt-3">
        {(['length', 'width', 'height'] as const).map((dimension) => {
          ...
          const labels = { length: 'Longitudinales', width: 'Transversales', height: 'Verticales' }
          return (
            <div key={dimension} className="text-center p-2 bg-muted rounded-xl">
              <div className="text-xs text-muted-foreground">{labels[dimension]}</div>
      ...
      {displayModes.filter(m => m.severity === "high").length > 0 && (
        <div className="mt-3 p-3 rounded-xl border border-destructive/20 bg-destructive/5">
          <h3 className="text-xs font-medium text-destructive mb-2">Modos críticos (requieren tratamiento):</h3>
