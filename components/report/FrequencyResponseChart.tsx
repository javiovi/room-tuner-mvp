  ...
  return (
    <div className="bg-card border border-border rounded-sm p-5 space-y-3">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
            ...
            <XAxis
              ...
              label={{ value: t.report.frequency.frequencyLabel, position: 'insideBottom', offset: -5, style: { fontSize: '10px', fill: theme.axis } }}
            />
            <YAxis
              domain={[-12, 12]} ticks={[-12, -6, 0, 6, 12]} stroke={theme.axis}
              style={{ fontSize: '10px', fill: theme.axis }}
              label={{ value: t.report.frequency.dbLabel, angle: -90, position: 'insideLeft', style: { fontSize: '10px', fill: theme.axis } }}
            />
            <Tooltip content={<CustomTooltip />} cursor={{ stroke: theme.primary, strokeWidth: 1 }} />
            <ReferenceLine y={0} stroke={theme.primary} strokeDasharray="3 3" strokeWidth={1} />
            <ReferenceLine y={6} stroke={theme.warning} strokeDasharray="2 2" strokeWidth={1} opacity={0.3} />
            <ReferenceLine y={-6} stroke={theme.warning} strokeDasharray="2 2" strokeWidth={1} opacity={0.3} />
            <Line type="monotone" dataKey="response" stroke={theme.primary} strokeWidth={2} dot={false} activeDot={{ r: 4, fill: theme.primary }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="flex items-center justify-center gap-4 text-xs text-muted-foreground pt-2 border-t border-border flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <div className="w-4 h-0.5 bg-warning border-t-2 border-dashed opacity-50"></div>
